...
$$
Y^TX=(Y^TY)^{\frac{1}{2}}\hat BD\hat A'(X^TX)^{\frac{1}{2}}
$$

That is to say, if we do SVD on $$Y^TX$$：

$$
Y^TX=\widetilde B D \widetilde A'
$$

$$
\hat B = (Y^TY)^{-\frac{1}{2}}\widetilde B ; \hat A=(X^TX)^{-\frac{1}{2}}\widetilde A
$$

$$
B=(Y^TY)^{-1}\widetilde B; A=(X^TX)^{-1}\widetilde A
$$
